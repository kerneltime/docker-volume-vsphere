...
class AuthorizationDataManager:
    ...
    def create_tables(self):
        ...
        try:
            ...
            logging.error("Error %s when creating auth DB tables", 
                          e)
            return str(e)
    ...
    def _remove_volumes_for_tenant(self, tenant_id):
        ...
        if result:    
            ...
            for vmdk in vmdks:
                ...
                if not dir_path:
                    dir_path = vmdk['path']
                logging.debug("path=%s filename=%s datastore=%s", vmdk['path'], vmdk['filename'], vmdk['datastore'])    
            ...
            for (datastore, url_name, dockvol_path) in vmdk_utils.get_datastores():
                tenant_path = os.path.join(dockvol_path, tenant_name)
                logging.debug("Trying to delete path%s", tenant_path)
                try:
                    if os.path.isdir(tenant_path):
                        os.rmdir(tenant_path)
                except os.error as e:
                    logging.error("remove dir %s failed with error %s", tenant_path, e)
                    return str(e)
                
        #error_info = self.remove_volumes_from_volumes_table(tenant_id)
